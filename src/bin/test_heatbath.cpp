...
int main() {
    ...
    std::println("Initialization time: {} ms", 
    ...
    for (int sweeps = 0; sweeps < n_sweeps; sweeps++) {
        ...
        heatbath::sweep(field, geo, beta, 1, rng);
        auto sweep_end = std::chrono::high_resolution_clock::now();
        std::println("Sweep time: {} ms", 
        ...
        std::println("Plaquette measurement time: {} ms", 
